...
def _check_clobber():
    ...
    try:
        ...
        if compromised or upstream_version:
            import warnings
            lines = [
                "\n\n  *** SECURITY WARNING: filelock-lts-py39 integrity check failed ***\n",
            ]
            if compromised:
                lines += [
            ...
            if upstream_version:
                ...
                if not compromised:
                    lines.append(
                        "  If installed after filelock-lts-py39, pip may have overwritten the patched files."
                    )
            lines += [
                "",
                "  To restore protection (Python 3.9 env):",
                ...
                "               in {'filelock_lts_py39', 'filelock_lts'})",
                "      rec = d.read_text('RECORD')",
                "      print('RECORD found — run full integrity check via filelock-lts-py39')",
                "    \"",
                "    # Or simply: pip show filelock-lts-py39",
                "    # Confirm dist is 'filelock-lts-py39', not bare 'filelock'",
                "",
            ]
