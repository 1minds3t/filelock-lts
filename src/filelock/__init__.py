...
def _check_clobber():
    ...
    installed after filelock-lts-py39, causing pip to overwrite _unix.py/_windows.py
    ...
    try:
        ...
        for dist in _md.distributions():
            ...
            if raw_name.lower() == "filelock" and raw_name not in LTS_NAMES:
                ...
                warnings.warn(
                    f"\n\n"
                    f"  *** SECURITY WARNING: filelock-lts-py39 may be compromised ***\n"
                    f"\n"
                    f"  Upstream 'filelock=={version}' is installed alongside this package.\n"
                    f"  If installed AFTER filelock-lts-py39, pip overwrote the patched\n"
                    ...
                    f"\n"
                    f"  To restore protection (Python 3.9 env):\n"
                    f"    pip uninstall filelock\n"
                    f"    pip install --force-reinstall filelock-lts-py39\n"
                    f"\n"
                    f"  Verify patch is active:\n"
                    f"    python -c \"import filelock; print(filelock.__version__)\"\n"
                    f"    # confirm dist is 'filelock-lts-py39', not 'filelock'\n",
                    stacklevel=2,
