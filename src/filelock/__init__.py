...
def _check_clobber():
    ...
    try:
        ...
        if compromised or upstream_version:
            import warnings
            lines = [
                "\n\n  *** SECURITY WARNING: filelock-lts-py37 integrity check failed ***\n",
            ]
            if compromised:
                lines += [
            ...
            if upstream_version:
                ...
                if not compromised:
                    lines.append(
                        "  If installed after filelock-lts-py37, pip may have overwritten the patched files."
                    )
            lines += [
                "",
                "  To restore protection (Python 3.7 env):",
                ...
                "               in {'filelock_lts_py37', 'filelock_lts'})",
                "      rec = d.read_text('RECORD')",
                "      print('RECORD found — run full integrity check via filelock-lts-py37')",
                "    \"",
                "    # Or simply: pip show filelock-lts-py37",
                "    # Confirm dist is 'filelock-lts-py37', not bare 'filelock'",
                "",
            ]
